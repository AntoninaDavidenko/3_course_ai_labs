...
w1 = np.random.randn(input_nodes, output_nodes)
...
for epoch in range(epochs):
    for j in range(len(dataset)):
        ...
        ef = correct_answer - h_node
        ef_delta = ef * sigmoid_derivative(h_node)
        #eh = np.dot(ef_delta, w2.T)
        #eh_delta = eh * sigmoid_derivative(h_node)

        #w2 += np.dot(h_node.T, ef_delta) * learning_rate
        w1 += np.dot(current_set.T, ef_delta) * learning_rate
